...
def fetch_and_save_spotify_data():
    url = "https://spotify-statistics-and-stream-count.p.rapidapi.com/artist/3tMTXQyRrPmMyHv5SoC0TV"
    headers = {
    ...
    if response.status_code == 200:
        data = response.json()
        with open('/opt/airflow/juicy_luicy_spotify.json', 'w') as json_file:
            json.dump(data, json_file, indent=4)
        print("Spotify data saved to juicy_luicy_spotify.json successfully.")
    else:
...
def fetch_and_save_youtube_data():
    ...
    querystring = {"id": "UCbctSuCowVV5nyQqlHSY9ZA", "hl": "en", "gl": "US"}
    ...
    if 'contents' in data:
        ...
        with open('/opt/airflow/juicy_luicy_youtube.json', 'w') as json_file:
            json.dump(channel_statistics, json_file, indent=4)

        print("Channel statistics saved to juicy_luicy_youtube.json")
...
def save_to_mongo(db, data):
    if data:
        collection = db['channel_statistics_juicy_luicy']
        
        existing_data = collection.find({})
        if collection.count_documents({}) > 0:  
            print("Existing data found, moving to historical collection...")
            historical_collection = db['historical_channel_statistics_juicy_luicy']
...
def upsert_spotify_data():
    ...
    with open('/opt/airflow/juicy_luicy_spotify.json', 'r') as json_file:
...
def save_youtube_data():
    ...
    with open('/opt/airflow/juicy_luicy_youtube.json', 'r') as json_file:
        data = json.load(json_file)
