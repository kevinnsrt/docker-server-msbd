...
def fetch_and_save_spotify_data():
    url = "https://spotify-statistics-and-stream-count.p.rapidapi.com/artist/06HL4z0CvFAxyc27GXpf02"
    headers = {
    ...
    if response.status_code == 200:
        data = response.json()
        with open('/opt/airflow/taylor_swift_spotify.json', 'w') as json_file:
            json.dump(data, json_file, indent=4)
        print("Spotify data saved to taylor_swift_spotify.json successfully.")
    else:
...
def fetch_and_save_youtube_data():
    ...
    querystring = {"id": "UCqECaJ8Gagnn7YCbPEzWH6g", "hl": "en", "gl": "US"}
    ...
    if 'contents' in data:
        ...
        with open('/opt/airflow/taylor_swift_youtube.json', 'w') as json_file:
            json.dump(channel_statistics, json_file, indent=4)

        print("Channel statistics saved to taylor_swift_youtube.json")
...
def save_to_mongo(db, data):
    if data:
        collection = db['channel_statistics_taylor_swift']
        
        existing_data = collection.find({})
        if collection.count_documents({}) > 0:  
            print("Existing data found, moving to historical collection...")
            historical_collection = db['historical_channel_statistics_taylor_swift']
...
def upsert_spotify_data():
    ...
    with open('/opt/airflow/taylor_swift_spotify.json', 'r') as json_file:
...
def save_youtube_data():
    ...
    with open('/opt/airflow/taylor_swift_youtube.json', 'r') as json_file:
        data = json.load(json_file)
