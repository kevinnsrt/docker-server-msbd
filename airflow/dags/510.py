...
def fetch_and_save_spotify_data():
    url = "https://spotify-statistics-and-stream-count.p.rapidapi.com/artist/298CtZa1wRSxNc38FmWbiX"
    headers = {
    ...
    if response.status_code == 200:
        data = response.json()
        with open('/opt/airflow/510_spotify.json', 'w') as json_file:
            json.dump(data, json_file, indent=4)
        print("Spotify data saved to 510_spotify.json successfully.")
    else:
...
def fetch_and_save_youtube_data():
    ...
    querystring = {"id": "UCQoW3oyEeUFNi--6qcvqZcg", "hl": "en", "gl": "US"}
    ...
    if 'contents' in data:
        ...
        with open('/opt/airflow/510_youtube.json', 'w') as json_file:
            json.dump(channel_statistics, json_file, indent=4)

        print("Channel statistics saved to 510_youtube.json")
...
def save_to_mongo(db, data):
    if data:
        collection = db['channel_statistics_510']
        
        existing_data = collection.find({})
        if collection.count_documents({}) > 0:  
            print("Existing data found, moving to historical collection...")
            historical_collection = db['historical_channel_statistics_510']
...
def upsert_spotify_data():
    ...
    with open('/opt/airflow/510_spotify.json', 'r') as json_file:
...
def save_youtube_data():
    ...
    with open('/opt/airflow/510_youtube.json', 'r') as json_file:
        data = json.load(json_file)
