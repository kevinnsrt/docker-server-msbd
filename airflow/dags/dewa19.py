...
def fetch_and_save_spotify_data():
    url = "https://spotify-statistics-and-stream-count.p.rapidapi.com/artist/48bKH1ugFBhERC1rdojP9d"
    headers = {
    ...
    if response.status_code == 200:
        data = response.json()
        with open('/opt/airflow/dewa19_spotify.json', 'w') as json_file:
            json.dump(data, json_file, indent=4)
        print("Spotify data saved to dewa19_spotify.json successfully.")
    else:
...
def fetch_and_save_youtube_data():
    ...
    querystring = {"id": "UC9mnKuwm9QVCOaFZpkqS_DQ", "hl": "en", "gl": "US"}
    ...
    if 'contents' in data:
        ...
        with open('/opt/airflow/dewa19_youtube.json', 'w') as json_file:
            json.dump(channel_statistics, json_file, indent=4)

        print("Channel statistics saved to dewa19_youtube.json")
...
def save_to_mongo(db, data):
    if data:
        collection = db['channel_statistics_dewa19']
        
        existing_data = collection.find({})
        if collection.count_documents({}) > 0:  
            print("Existing data found, moving to historical collection...")
            historical_collection = db['historical_channel_statistics_dewa19']
...
def upsert_spotify_data():
    ...
    with open('/opt/airflow/dewa19_spotify.json', 'r') as json_file:
...
def save_youtube_data():
    ...
    with open('/opt/airflow/dewa19_youtube.json', 'r') as json_file:
        data = json.load(json_file)
