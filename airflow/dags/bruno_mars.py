...
def fetch_and_save_spotify_data():
    ...
    if response.status_code == 200:
        data = response.json()
        with open('/opt/airflow/bruno_mars_spotify.json', 'w') as json_file:
            json.dump(data, json_file, indent=4)
        print("Spotify data saved to bruno_mars_spotify.json successfully.")
    else:
...
def fetch_and_save_youtube_data():
    ...
    querystring = {"id": "UCoUM-UJ7rirJYP8CQ0EIaHA",}
    ...
    if "data" in data and isinstance(data["data"], list):
        ...
        with open('/opt/airflow/bruno_mars_youtube.json', 'w') as json_file:
            json.dump(channel_statistics, json_file, indent=4)

        print("Channel statistics saved to bruno_mars_youtube.json")
...
def save_to_mongo(db, data):
    if data:
        collection = db['channel_statistics_bruno_mars']
        
        existing_data = collection.find({})
        if collection.count_documents({}) > 0:  
            print("Existing data found, moving to historical collection...")
            historical_collection = db['historical_channel_statistics_bruno_mars']
...
def save_youtube_data():
    ...
    with open('/opt/airflow/bruno_mars_youtube.json', 'r') as json_file:
        data = json.load(json_file)
